...
public class ResourceDrawer {
	private NewSystemVeil newSystemVeil = new NewSystemVeil();
	/**
	 * クラスコンストラクタ．
	 */
	...
	public void drawResource(Character[] characters, Deque<LoopEffect> projectiles,
		...
		drawVeilImage1(remainingTime, GraphicManager.getInstance().getScreenImage(), characters, shouldRender);
	...
	public void drawVeilImage(int time, BufferedImage bufferedImage, Character[] characters, boolean shouldRender) {
		int diameter = 0;
		int maxDiameter = 300;
		int interval = 500;
		if (time > 60000 - maxDiameter - interval){
			diameter = (int)(time - 60000 + maxDiameter + interval);
		...
		for(int i = 0; i < characters.length-1; i++) {
			int x = (characters[i].getHitAreaCenterX())-diameter/2;
			int y = (characters[i].getHitAreaCenterY())-diameter/2;
			area.add(new Area(new Ellipse2D.Float(x, y, diameter, diameter)));
		...
		// 	int x = (characters[i].getHitAreaCenterX())-100;
		// 	int y = (characters[i].getHitAreaCenterY())-100;
		// 	g2d.drawOval(x, y, diameter/2, diameter/2);
		// }
	...
	public void drawVeilImage1(int time, BufferedImage bufferedImage, Character[] characters, boolean shouldRender) {
		newSystemVeil.initLightArea(time, characters[0]);
		newSystemVeil.setArea();
		Image veilImage = newSystemVeil.drawVeilImage(time, bufferedImage, shouldRender);
		GraphicManager.getInstance().drawImage(veilImage, 0, 0, Image.DIRECTION_RIGHT, shouldRender);
